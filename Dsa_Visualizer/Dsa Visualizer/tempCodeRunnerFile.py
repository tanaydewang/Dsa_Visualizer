...
while running:
    ...
    for event in pygame.event.get():
        ...
        if current_state != "main_menu" and handle_button_event(back_button):
            state_manager.pop_state(); animation_generator=None; status_message=""; highlighted_line = -1
        
        if animation_generator:
            if handle_button_event(pause_button):
                is_paused = not is_paused
            if handle_button_event(step_button) and is_paused:
                try:
                    result=next(animation_generator)
                    if isinstance(result, dict):
                        status_message = result.get('msg', status_message)
                        highlighted_line = result.get('line', highlighted_line)
                except StopIteration:
                    animation_generator=None; status_message="Finished!"
        ...
        if current_state == "main_menu":
            ...
            def select_sort(sort_func):
                global sort_function_to_run; sort_function_to_run=sort_func; sorting_viz.generate_array(); state_manager.push_state("sorting_visualizer")
        ...
        if current_state.endswith("_visualizer"):
            ...
            if current_state == "sorting_visualizer":
                ...
                binary_search_button = Button(600,650,150,50,"Search",main_font,('action','hover_action')); binary_search_button.update_theme(THEMES[current_theme])
                ...
                array_search_button = Button(940,650,150,50,"Search",main_font,('accent1','hover1')); array_search_button.update_theme(THEMES[current_theme])
                ...
                trie_search_button = Button(770,650,150,50,"Search",main_font,('accent1','hover1')); trie_search_button.update_theme(THEMES[current_theme])
    ...
    if current_state == "main_menu":
        ds_button.draw(screen); algo_button.draw(screen); settings_button.draw(screen); help_button.draw(screen)
    elif current_state == "ds_menu":
        tree_button.draw(screen); queue_button.draw(screen); stack_button.draw(screen); ll_button.draw(screen); heap_button.draw(screen); array_button.draw(screen); trie_button.draw(screen); back_button.draw(screen)
    elif current_state == "tree_menu":
        bst_button.draw(screen); rbt_button.draw(screen); back_button.draw(screen)
    elif current_state == "algo_menu":
        sorting_algo_button.draw(screen); searching_algo_button.draw(screen); back_button.draw(screen)
    elif current_state == "sorting_menu":
        selection_sort_button.draw(screen)
        bubble_sort_button.draw(screen)
        insertion_sort_button.draw(screen)
        back_button.draw(screen)
    elif current_state == "settings_menu":
        theme_button.draw(screen); sound_button.draw(screen); back_button.draw(screen)
    ...
    elif current_state == "bst_visualizer":
        bst_viz.draw(screen, main_font); input_box_value.draw(screen); insert_button.draw(screen); back_button.draw(screen)
    elif current_state == "rbt_visualizer":
        rbt_viz.draw(screen); input_box_value.draw(screen); insert_button.draw(screen); back_button.draw(screen)
    elif current_state == "queue_visualizer":
        queue_viz.draw(screen); input_box_value.draw(screen); enqueue_button.draw(screen); dequeue_button.draw(screen); back_button.draw(screen)
    elif current_state == "stack_visualizer":
        stack_viz.draw(screen); input_box_value.draw(screen); push_button.draw(screen); pop_button.draw(screen); back_button.draw(screen)
    elif current_state == "ll_visualizer":
        ll_viz.draw(screen); input_box_value.draw(screen); insert_head_button.draw(screen); delete_button.draw(screen); back_button.draw(screen)
    elif current_state == "heap_visualizer":
        heap_viz.draw(screen); input_box_value.draw(screen); insert_button.draw(screen); extract_min_button.draw(screen); back_button.draw(screen)
    elif current_state == "array_visualizer":
        array_search_button = Button(940,650,150,50,"Search",main_font,('accent1','hover1')); array_search_button.update_theme(THEMES[current_theme])
        ...
        code_panel = CodePanel(SCREEN_WIDTH - 380, 100, 360, SCREEN_HEIGHT - 250, sorting_viz.PSEUDOCODE, help_font, theme_colors)
    ...
    if animation_generator:
        pause_button.text=">" if is_paused else "||"; pause_button.draw(screen); step_button.draw(screen)
    if current_state.endswith("_visualizer"):
